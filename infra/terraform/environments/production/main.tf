...
variable "aws_linux_ami_id" {
  description = "Base AMI for production non-GPU nodes."
  type        = string
...
variable "edge_site_count" {
  description = "Number of production edge sites."
  type        = number
  default     = 2
}
...
locals {
  counts = {
    ...
    monitoring    = 1
    services      = 2
    edge_gateways = var.edge_site_count
    nats          = var.edge_site_count
    mlflow        = 1
  }
